# DAG_Validation test 
# correspond to the tests will be uniformly applied to all DAGs

# To execute this test, get docker exec -it "DOCKER ID" /bin/bash to open Airlfow CLI
# "cd home/test_dags", pytest test_dag_validation.py -v
...
class TestDagValidation:
    ...
    def test_time_import_dags(self, dagbag):
        ...
        # In order to get statistics of the loaded DAGs from DagBag object
        slow_dags = list(filter(lambda f: f.duration > self.LOAD_SECOND_THRESHOLD, stats))
        # compare DAG's load duration to LOAD_SECOND_THRESHOLD where defined at class 
    ...
    @pytest.mark.skip(reason="not yet added to the DAGs") # marked as skip to not executed test
    def test_default_args_email(self, dagbag):
        ...
        for dag_id, dag in dagbag.dags.items():
            emails = dag.default_args.get('email', []) # In order to get default argument of the loaded DAGs from DagBag object
